...
CREATE INDEX idx_buyers_company_name ON buyers(company_name);
...
-- Function to update supplier listing count
CREATE OR REPLACE FUNCTION update_supplier_total_listings()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        SET total_listings = (
            ...
            WHERE supplier_id = NEW.supplier_id AND is_active = TRUE
        )
    ...
    ELSIF TG_OP = 'DELETE' THEN
        ...
        SET total_listings = (
            SELECT COUNT(*) 
            FROM supply_listings 
            WHERE supplier_id = OLD.supplier_id AND is_active = TRUE
        )
...
-- Trigger to update supplier total listings
CREATE TRIGGER update_supplier_listings_count
    AFTER INSERT OR UPDATE OR DELETE ON supply_listings
    FOR EACH ROW EXECUTE FUNCTION update_supplier_total_listings();

-- Function to update buyer requirements count
CREATE OR REPLACE FUNCTION update_buyer_total_requirements()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' THEN
        UPDATE buyers 
        SET total_requirements = (
            ...
            WHERE buyer_id = NEW.buyer_id AND is_active = TRUE
        )
    ...
    ELSIF TG_OP = 'DELETE' THEN
        ...
        SET total_requirements = (
            SELECT COUNT(*) 
            FROM demand_listings 
            WHERE buyer_id = OLD.buyer_id AND is_active = TRUE
        )
...
-- Trigger to update buyer total requirements
CREATE TRIGGER update_buyer_requirements_count
    AFTER INSERT OR UPDATE OR DELETE ON demand_listings
    FOR EACH ROW EXECUTE FUNCTION update_buyer_total_requirements();
...
ALTER TABLE demand_listings ENABLE ROW LEVEL SECURITY;
ALTER TABLE marketplace_contacts ENABLE ROW LEVEL SECURITY;

-- Suppliers can only view and edit their own profile
CREATE POLICY "Suppliers can view own profile" ON suppliers FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Suppliers can update own profile" ON suppliers FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Anyone can view verified suppliers" ON suppliers FOR SELECT USING (verified = TRUE);

-- Buyers can only view and edit their own profile
CREATE POLICY "Buyers can view own profile" ON buyers FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Buyers can update own profile" ON buyers FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Anyone can view verified buyers" ON buyers FOR SELECT USING (verified = TRUE);

-- Supply listings policies
CREATE POLICY "Anyone can view active supply listings" ON supply_listings FOR SELECT USING (is_active = TRUE);
CREATE POLICY "Suppliers can manage own listings" ON supply_listings FOR ALL USING (auth.uid() = supplier_id);

-- Demand listings policies
CREATE POLICY "Anyone can view active demand listings" ON demand_listings FOR SELECT USING (is_active = TRUE);
CREATE POLICY "Buyers can manage own listings" ON demand_listings FOR ALL USING (auth.uid() = buyer_id);

-- Marketplace contacts policies
CREATE POLICY "Users can view own contacts" ON marketplace_contacts FOR SELECT USING (auth.uid() = requester_id);
CREATE POLICY "Users can create contacts" ON marketplace_contacts FOR INSERT WITH CHECK (auth.uid() = requester_id);
CREATE POLICY "Users can update own contacts" ON marketplace_contacts FOR UPDATE USING (auth.uid() = requester_id);

-- ===============================================
-- COMMENTS
-- ===============================================

COMMENT ON TABLE suppliers IS 'Suppliers/vendors who provide raw materials';
COMMENT ON TABLE buyers IS 'Buyers/manufacturers who purchase raw materials';
COMMENT ON TABLE supply_listings IS 'Materials available for sale by suppliers';
COMMENT ON TABLE demand_listings IS 'Material requirements posted by buyers';
COMMENT ON TABLE marketplace_contacts IS 'Contact requests between buyers and suppliers';
